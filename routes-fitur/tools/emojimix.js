...
router.get('/', async (req, res) => {
  ...
  try {
    const apiRes = await fetch(`https://tenor.googleapis.com/v2/featured?key=LIVDSRZULELA&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${encodeURIComponent(emoji1)}_${encodeURIComponent(emoji2)}`);
    if (!apiRes.ok) {
